...
public class PolynomialSolver implements IPolynomialSolver {
        ...
        var1 = sort(chooseList(poly));
            for (int i = 0; i < var1.size(); i += 2) {
                if ((int) var1.get(i) > 0) {
                    ...
                    ///////
                    if ((int) var1.get(i) != 1)
                        polynomial = polynomial.concat(var1.get(i).toString()); // to prevent // 1X - 1
                    
                    if((int) var1.get(i)==1 && (int) var1.get(i+1)==0)
                    	 polynomial = polynomial.concat(var1.get(i).toString());
                    ////////
                } else if ((int) var1.get(i) < 0)
                    if ((int) var1.get(i) != -1)
                        polynomial = polynomial.concat(var1.get(i).toString());
                    else
                        polynomial = polynomial.concat("-");
                if ((int) var1.get(i) != 0 && (int)var1.get(i+1)!=0) {// to prevent X^0
                    polynomial = polynomial.concat("X");
                    if ((int)var1.get(i+1)!=1)
                        polynomial = polynomial.concat("^" + var1.get(i + 1).toString()); // to prevent X^1
                }
            }
    ...
    for(int i = 0;i<R.size();i+=2) {            // to remove terms with zero coefficient
        if((int)R.get(i)==0) {
            R.remove(i);
            R.remove(i);
        }
    ...
    ///////////////////////////////////////////////////////// for testing
    public int[] getcol (int [][] array , int column){
